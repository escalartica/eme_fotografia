...
**Content note (corrected 2026-09-01, ruling recorded in this plan's ledger before Task 6 was dispatched):** `content/services.ts` has no per-service image field today. Add one (`Service.previewImage?: string`). The original draft of this note assumed a "real wedding cover photo" exists to use for the `boda` service — checked directly: it doesn't. Every photo-type project cover in `content/projects.ts` (`clara-y-manuel`, `lucia-y-jorge`) is `isPlaceholderMedia: true`, sourced from Unsplash — the ONLY genuinely real (non-placeholder) client asset in the seed data is `boda-real-01`'s video and its poster frame. So:
- `video` service → `/videos/posters/real-boda-01-full.webp` (genuinely real, the same poster already reused in Task 1's Hero and Task 9's masked-wordmark moment — reinforces the recurring-real-asset motif again).
- `boda` service → `/images/trabajos/clara-y-manuel/placeholder-cover.webp` (the existing PLACEHOLDER wedding cover, already publicly shown elsewhere on the site — reusing it here introduces no new stock imagery and no new dishonesty, since it's already displayed as-is on `/trabajos` and the Home `SelectedWork` grid; it is simply not a "real" photo the way the video is, and this task doesn't claim otherwise).
- `fotomaton`/`360` → leave `previewImage` undefined (no matching project exists yet, Task 8 of the incremental plan is still parked on this) — design the hover-reveal to degrade gracefully when absent (e.g. show nothing, or a neutral placeholder box), same honesty pattern as `Testimonial.photo?` from the incremental plan's Task 13.

- [ ] **Step 1:** Add `previewImage?: string` to `Service` in `content/types.ts`. In `content/services.ts`, set `video: '/videos/posters/real-boda-01-full.webp'` and `boda: '/images/trabajos/clara-y-manuel/placeholder-cover.webp'` (both verified paths, use verbatim — do not guess or re-derive them). Leave `fotomaton`/`360` without the field.
...
**Structural correction (2026-09-01, ruling recorded in this plan's ledger, found before dispatch by direct testing of the exact same pattern in Task 7):** `Manifiesto.tsx` currently wraps its whole `<section>` in an outer `<ScrollReveal>` (a one-shot, fixed-duration `y: 40 → 0` translate + fade-in, triggered at `'top 85%'`). Task 7's implementation nested a second, independent scroll-triggered element (also inside a `ScrollReveal`) as a DESCENDANT of that same translating wrapper — and direct real-browser testing proved this actually breaks: the ancestor's translateY shifts the descendant's DOM position while the descendant's own ScrollTrigger simultaneously measures/animates against that same shifting position, and the animation got stuck indefinitely mid-transition (confirmed via `getComputedStyle`: `opacity: 0.68`, `filter: blur(2px)`, never settling). This task's new `scrub: true` masked-layer trigger would have the same structural problem if nested inside the existing outer `ScrollReveal` (Step 3 as originally drafted asked for exactly this). **Fix: remove the outer `<ScrollReveal>` wrapper from `Manifiesto.tsx` entirely.** The new masked-photo crossfade itself already provides a "reveal as this section enters the viewport" moment for the whole section — a separate, redundant entrance fade is no longer needed once this task lands, and removing it eliminates the nesting conflict at the source rather than working around it. The section's `<h2>`/`<p>` render as plain elements (no wrapping `ScrollReveal`, no `'use client'` needed for that reason — though the component still needs `'use client'` for the new GSAP effect itself, see below).

- [ ] **Step 1: Write the failing test**

`Manifiesto.test.tsx` currently has no `gsap`/`ScrollTrigger` mocks at all — it didn't need them before, since the GSAP work lived inside the separately-tested `ScrollReveal` component, not in `Manifiesto` itself. Now that this task moves a GSAP effect directly into `Manifiesto.tsx`, add `vi.mock('gsap', () => ({ gsap: { to: vi.fn(), context: vi.fn((fn) => { fn(); return { revert: vi.fn() }; }), registerPlugin: vi.fn() } }))` and `vi.mock('gsap/ScrollTrigger', () => ({ ScrollTrigger: {} }))` to the top of the test file, matching `Hero.test.tsx`'s exact established mock shape for this project's `gsap.context`-based effects (read that file's mocks before writing these, don't guess the shape).

Extend `Manifiesto.test.tsx`: assert the section renders TWO copies of the heading text (the base layer + the masked layer — use a query that can distinguish them, e.g. by a `data-testid` on the masked layer), and assert the masked layer's element has `aria-hidden="true"` (it's a pure visual duplicate of the same text — screen readers must only encounter the heading once, via the base layer, to avoid the text being announced twice). Keep the existing test's assertions passing too — the heading/copy content itself is unchanged.
...
```
In `Manifiesto.tsx`: remove the outer `<ScrollReveal>` wrapper per the structural correction above (the `<section>` becomes the top-level element the component returns, no longer a `ScrollReveal` child — `ScrollReveal`'s own import can be removed if nothing else in the file uses it). Add `'use client'` to the top of the file (needed for the new `useEffect`/GSAP work below — `Manifiesto` currently has no directive because it was a plain server-renderable component wrapped by a client `ScrollReveal`; now the GSAP effect lives directly in this component, so it needs the directive itself). Wrap the heading in `.headingWrap`, keep the real `<h2>` as `.headingBase` (unchanged text, unchanged semantics — this is what screen readers and SEO see), and add a second `aria-hidden="true"` element (a `<span>` or duplicate non-semantic element, NOT a second `<h2>` — only one real heading per section) with the identical text and `.headingMasked` class. In a `useEffect`, set up a `gsap.to(maskedRef.current, { opacity: 1, scrollTrigger: { trigger: sectionRef.current, start: 'top 70%', end: 'top 20%', scrub: true } })`, gated behind `useReducedMotion()` exactly like `Hero.tsx`'s Task-21 parallax pattern (read that code again for the exact `gsap.context`/cleanup shape to reuse) — `sectionRef` is a new ref on the `<section>` element itself (now a plain top-level element in this component, not nested inside anything that also animates its own position, per the structural correction above).
...
- [ ] **Step 2:** Emulate `prefers-reduced-motion: reduce` in the browser and walk through every route touched by Tasks 1–9, confirming every new animation (Hero video/parallax, header hide/show, filter re-flow, services hover-reveal, Manifiesto's masked-photo wordmark crossfade — NOT "pinned horizontal scroll", that idea was superseded by the masked-photo reveal before Task 9 was dispatched, see that task's own correction note) correctly falls back to its static/instant equivalent — not just that it doesn't crash, that it's still a complete, usable experience.
